...
export function getSettlementByRecipeId(recipeId) {
    const db = getDatabase();
    ...
    LIMIT 1
  `);
    const row = stmt.get(recipeId, `%"recipeId":"${recipeId}"%`);
    if (!row) {
...
export function createOrUpdatePosition(position) {
    const db = getDatabase();
    const now = Date.now();
    const stmt = db.prepare(`
  ...
  `);
    stmt.run(position.positionId, position.poolAddress, position.depositAmount, position.chain, position.rpcUrl ?? null, position.status, now, now);
...
export function getPositionByPoolAddress(poolAddress, chain) {
    const db = getDatabase();
    ...
    const stmt = chain
      ...
      `);
    const row = (chain ? stmt.get(normalizedPool, chain) : stmt.get(normalizedPool));
    return row ? rowToPosition(row) : null;
}
 ...
 */
export function addToPositionOrCreate(params) {
...
export function getActivePositions(limit = 100) {
    const db = getDatabase();
    const stmt = db.prepare(`
...
export function updatePositionPool(positionId, nextPoolAddress) {
    const db = getDatabase();
    const now = Date.now();
    const stmt = db.prepare(`
    UPDATE positions
...
export function createMonitoringReport(report) {
    const db = getDatabase();
    const now = Date.now();
    const stmt = db.prepare(`
  ...
  `);
    stmt.run(report.reportId, report.positionId, report.poolAddress, report.depositAmount, report.currentLiquidity, report.status, report.nextBestPool ?? null, report.nextBestLiquidity ?? null, report.reason, report.chain, report.timestamp, report.executionStatus ?? null, report.executionTxHash ?? null, report.executionError ?? null, now, now);
    return getMonitoringReport(report.reportId);
}
export function updateMonitoringReportExecution(reportId, status, txHash, error) {
    const db = getDatabase();
    const now = Date.now();
    const stmt = db.prepare(`
...
export function getMonitoringReport(reportId) {
    const db = getDatabase();
    const stmt = db.prepare(`
...
export function getMonitoringReports(limit = 100) {
    const db = getDatabase();
    const stmt = db.prepare(`
...
export function getLatestMonitoringReportByPosition(positionId) {
    const db = getDatabase();
    const stmt = db.prepare(`
    SELECT * FROM monitoring_reports
    WHERE position_id = ?
...
export function getActivePositionsWithMonitoring(limit = 100) {
    const db = getDatabase();
    const stmt = db.prepare(`
    SELECT
      p.position_id,
  ...
  `);
    ...
    return rows.map((row) => ({
        ...
        latestMonitoringStatus: row.latest_monitoring_status ??
            undefined,
        latestLiquidity: row.latest_liquidity ?? undefined,
        lastScanAt: row.last_scan_at ?? undefined,
    }));
}
export function setTelegramAlerts(chatId, enabled) {
    const db = getDatabase();
    const now = Date.now();
    const stmt = db.prepare(`
...
export function getTelegramAlertSetting(chatId) {
    const db = getDatabase();
    const stmt = db.prepare(`
...
export function getEnabledTelegramChatIds() {
    const db = getDatabase();
    const stmt = db.prepare(`
...
function rowToMonitoringReport(row) {
    return {
