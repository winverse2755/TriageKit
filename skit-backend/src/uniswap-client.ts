...
type UniswapQuoteResponse = {
  ...
  route?: Array<string | { tokenOutSymbol?: string; tokenOut?: string }>;
  tx?: {
    to?: string;
    data?: string;
    value?: string;
  ...
  return value
    .map((hop) => {
      ...
      if (hop && typeof hop === "object") {
        const out = (hop as { tokenOutSymbol?: string; tokenOut?: string })
          .tokenOutSymbol;
        if (out) return out;
        const tokenOut = (hop as { tokenOut?: string }).tokenOut;
        return tokenOut ?? "";
      }
      return "";
    })
...
export async function quoteUniswapRotation(args: {
  ...
  const baseUrl = (
    process.env.UNISWAP_API_BASE_URL ?? "https://api.uniswap.org/v1"
  ).replace(/\/$/, "");
  ...
  try {
    ...
    const amountOut =
      amountOutRaw !== undefined ? String(amountOutRaw) : undefined;
    ...
    console.warn(
      "[Uniswap] Quote failed, using deterministic fallback quote:",
